...
private val MainHandles = listOf(
    Offset(0f, 0f), Offset(1f, 1f), Offset(1f, 0f), Offset(0f, 1f)
)

private val SecondaryHandles = listOf(
    Offset(.5f, 0f), Offset(1f, .5f), Offset(.5f, 1f), Offset(0f, .5f)
)
...
public fun CropperStyle(
    ...
    override val handleColor: Color
        get() = handleColor
    ...
    override val handleStrokeWidth: Dp get() = handleStrokeWidth
    override val touchRad: Dp
        get() = touchRad
    ...
    override fun DrawScope.drawCropRect(region: Rect) {
        ...
        drawHandles(finalRegion, handleStrokeWidth, handleColor, touchRad)
    }
    ...
    private fun DrawScope.drawHandles(
        ...
        handleColor: Color,
        handleStrokeThickness: Dp
    ) {
        val strokeWidth = handleStrokeWidth.toPx()

        val path = Path().apply {
            if (region != Rect.Zero) {
                region.let { rect ->
                    // Top left lines
                    moveTo(rect.topLeft.x, rect.topLeft.y + strokeWidth)
                    lineTo(rect.topLeft.x, rect.topLeft.y)
                    lineTo(rect.topLeft.x + strokeWidth, rect.topLeft.y)

                    // Top right lines
                    moveTo(rect.topRight.x - strokeWidth, rect.topRight.y)
                    lineTo(rect.topRight.x, rect.topRight.y)
                    lineTo(rect.topRight.x, rect.topRight.y + strokeWidth)

                    // Bottom right lines
                    moveTo(rect.bottomRight.x, rect.bottomRight.y - strokeWidth)
                    lineTo(rect.bottomRight.x, rect.bottomRight.y)
                    lineTo(rect.bottomRight.x - strokeWidth, rect.bottomRight.y)

                    // Bottom left lines
                    moveTo(rect.bottomLeft.x + strokeWidth, rect.bottomLeft.y)
                    lineTo(rect.bottomLeft.x, rect.bottomLeft.y)
                    lineTo(rect.bottomLeft.x, rect.bottomLeft.y - strokeWidth)
                }
            }
        }

        drawPath(
            path, color = handleColor, style = Stroke(
                width = handleStrokeThickness.toPx(),
...
//        }
    ...
    private fun DrawScope.drawGuidelines(
        guidelines: CropperStyleGuidelines, region: Rect
...
private inline fun DrawScope.clipRect(
    rect: Rect, op: ClipOp = ClipOp.Intersect, block: DrawScope.() -> Unit
) {
    clipRect(
        left = rect.left,
        top = rect.top,
